...
function boundBox(letter,imageSel){
	...
	if(modelNo==1){//if collagen
		if(letter=='a'){
			...
			if(a.checked){jmolScript('boundbox  ((:a and 91-133) or (:a and 324-366) or (:a and 557-599) or (:a and 793-835) or (:a and 1027-1054)) 2; draw a'+imageSel+' boundbox mesh nofill;boundbox off; color $a'+imageSel+' yellow;',imageSel);}
			else if(a.checked==false){jmolScript('delete $a'+imageSel+'',imageSel);}
		}
		else if(letter=='b'){
			var b=document.getElementById('b'+imageSel);
			if(b.checked){jmolScript('boundbox  ((:a and 147-174) or (:a and 380-407) or (:a and 613-640) or (:a and 849-876)) 2; draw e'+imageSel+' boundbox mesh nofill;boundbox off; color $e'+imageSel+' blue;',imageSel);}
			else if(e.checked==false){jmolScript('delete $e'+imageSel+'',imageSel);}		
		}
		else if(letter=='c'){
		}
		else if(letter=='d'){
		...
		//	if(e.checked){jmolScript('boundbox  ((:a and 147-174) or (:a and 380-407) or (:a and 613-640) or (:a and 849-876)) 2; draw e'+imageSel+' boundbox mesh nofill;boundbox off; color $e'+imageSel+' blue;',imageSel);}
		//	else if(e.checked==false){jmolScript('delete $e'+imageSel+'',imageSel);}		
		}
	}
...
function imageStyle(imageSel){	
	...
	if(BallnStick.checked){jmolScript('select all;spacefill '+aSize[imageNo]+'%; wireframe '+bSize[imageNo]+'',imageSel)}
	else if(BallnStick.checked==false){jmolScript("select all;spacefill off;wireframe off",imageSel)}
	
	if(Trace.checked){jmolScript('select *.ca;trace;color trace orange',imageSel)}
	else if(Trace.checked==false){jmolScript("select all;trace off",imageSel)}

	if(Rib.checked){jmolScript('select all;ribbon;color ribbon "#FFFF66"',imageSel)}
	else if(Rib.checked==false){jmolScript("select all;ribbon off",imageSel)}
}
...
{	var imageNo=0;
	...
	if(BallnStick.checked){jmolScript('select all;spacefill '+aSize[imageNo]+'%',imageSel)}
	else if(BallnStick.checked==false){jmolScript("select all;spacefill off",imageSel)}
	return aSize;
}
...
{	var imageNo=0;
	...
	if(BallnStick.checked){jmolScript('select all;wireframe '+bSize[imageNo]+'%',imageSel)}
	else if(BallnStick.checked==false){jmolScript("select all;wireframe off",imageSel)}
	return bSize;
}

function Spin(imageSel)
{	jmolScript("if(_spinning);spin off;spinflag = false;else;spin on;spinflag = true;endif",imageSel);
}
