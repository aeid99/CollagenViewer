...
function customhoverL(){
	...
	var hoverMsgVal=0;
	var searchHover=0;
	Jmol.script(myJmol,"select all;halos off;hover off","Left");
	var i=1;
	...
	for(k=1;k<=sitesShown[0];k++){
		...
		if(searchedSite&&(document.getElementById("searchLeft").value!="")){
			...
			Jmol.script(myJmol,'select siteSearchLeft;halos 45%;color halos yellow;select all','Left');
		}
	}
		
	if(sitesPickedL[0]==1 && searchHover==0){
		Jmol.script(myJmol,"select all; halos off;hover %U","Left");
	}
	if(sitesPickedL[0]==1 && searchHover==1){
		Jmol.script(myJmol,'select siteSearchLeft;halos 45%;color halos yellow;hover '+hoverMsg+';select all','Left');
	}
	else{
		while(i < sitesPickedL.length){
			...
			//goes through every site and determines if it is being hovered..
				...
				if(boolSite){
					...
					Jmol.script(myJmol,'select site'+i+';halos 45%;color halos '+hoverColorL[i]+';select all',"Left");
				}
			}
			i=i+1;
		}
		Jmol.script(myJmol,'hover '+hoverMsg,"Left");
	}
}
